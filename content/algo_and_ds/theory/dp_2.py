...
def count_paths_memoization(m, n):
    ...
    def explore(row, col, memo):
        ...
        total_paths = explore(row + 1, col, memo) + explore(row, col + 1, memo)
        memo[(row, col)] = total_paths
        return memo[(row, col)]

    # Start the exploration from the top-left corner (0, 0)
    memo = {}
    return explore(0, 0, memo)
...
total_paths = count_paths_memoization(m_rows, n_cols)
